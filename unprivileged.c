...
void do_sqfs_mount(const char image[], int offset, const char mountpoint[]) {
  ...
  if (pid == 0) {
    ...
    if (!err) {
      ...
      sqfs_ll_chan ch;
      err = -1;
      if (sqfs_ll_mount(&ch, mountpoint, &args, &sqfs_ll_ops,
                        sizeof(sqfs_ll_ops), ll) == SQFS_OK) {
